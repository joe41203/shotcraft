...
description: >
  ...
  Mochiy Pop One in lowercase weight 900 treated as graphic primitive, IBM Plex Mono chrome (uppercase,
...
colors:
  canvas: "#0b0f19"   # ベースの地。assemble-index はこの canvas ロールを #root に塗る
  ink-black: "#0b0f19"
  ink-black-alt: "#161b26"
  fire-orange: "#10b981"
  cream: "#f2f5f9"
  cream-muted: "#9aa6b8"
  cream-hint: "#65707e"
  border-dark: "#1f2632"
  ink-on-orange-muted: "rgba(4,20,13,0.78)"
  ink-on-orange-hint: "rgba(4,20,13,0.58)"
  ink-on-orange-faint: "rgba(4,20,13,0.42)"
  ink-on-orange-border: "rgba(4,20,13,0.22)"
...
typography:
  ...
  # — display / hero ramp (Noto Sans JP 700; JA copy so no lowercase transform) —
  h3:      { fontFamily: "Noto Sans JP", cqw: 2.8, weight: 700, lineHeight: 1.2 }
  quote-text:{ fontFamily: "Noto Sans JP", cqw: 3.8, weight: 700, lineHeight: 1.15, tracking: "-0.02em" }
  h2:      { fontFamily: "Noto Sans JP", cqw: 4.5, weight: 700, lineHeight: 1.1, tracking: "-0.02em" }
  stat-value:{ fontFamily: "Noto Sans JP", cqw: 5.5, weight: 700, lineHeight: 1.0, tracking: "-0.04em" }
  h1:      { fontFamily: "Noto Sans JP", cqw: 7.5, weight: 700, lineHeight: 0.9, tracking: "-0.03em" }
  fadelist-item:{ fontFamily: "Noto Sans JP", cqw: 7.5, weight: 700, lineHeight: 1.0, tracking: "-0.03em" }
  quote-mark:{ fontFamily: "Noto Sans JP", cqw: 10.0, weight: 700, lineHeight: 0.6 }
  fadelist-title:{ fontFamily: "Noto Sans JP", cqw: 10.5, weight: 700, lineHeight: 0.9, tracking: "-0.04em" }
  display: { fontFamily: "Noto Sans JP", cqw: 13.0, weight: 700, lineHeight: 0.88, tracking: "-0.04em" }
...
- **Fonts** — already set to **Mochiy Pop One** (display) / **system-ui** (body); ignore any preset font name lingering in prose.
...
text and becomes graphic primitive.** Mochiy Pop One `display` at 13cqw puts a single lowercase word
...
**Mochiy Pop One** carries every text role from display to body — expressive range from weight (400–900)
...
- **Massive lowercase Mochiy Pop One 900**, negative-tracked, as graphic primitive (display 13cqw).
...
Two ramps. The **reading ramp** (Mochiy Pop One body 1.2cqw, mono label 0.72cqw) carries copy + chrome; the
**display ramp** (Mochiy Pop One `h2` 4.5cqw → `display` 13cqw, weight 700–900) carries every statement.
...
- **Mochiy Pop One display is lowercase, weight 700–900, negative-tracked** (−0.04em largest, −0.02em h2). **Mono chrome is uppercase, 0.1em+.** No italic, no underline, no uppercase display.
...
Mochiy Pop One `display` (13cqw) lowercase in ink, left-anchored, over a small ink rule stub + mono kicker; a
Mochiy Pop One lead line beneath in 75% ink. **Chrome** mono catalogue number top-left, mono meta top-right
...
**Ground** ink-black. **Composes** kicker, display. **Focal** a 2–4 word Mochiy Pop One `display`/`h1`
...
row of three top-border-only stat-cards — big Mochiy Pop One-900 numeral in `{colors.fire-orange}`, Mochiy Pop One
...
**Ground** ink-black. **Composes** fadelist (3 stacked Mochiy Pop One-900 words at 1.0/0.5/0.22), fadelist-title.
...
Mochiy Pop One `quote-text` (700, lowercase) at ≤78cqw under an oversized fire-orange `quote-mark` (10cqw,
...
- Set every Mochiy Pop One display in **lowercase weight 900**, negative-tracked — the system's signature.
...
- Never uppercase Mochiy Pop One display; never add a second accent color.
...
- No serif companion; chrome is never Mochiy Pop One.
...
- **Type** — Mochiy Pop One lowercase 900 negative-tracked, fit-to-measure; mono chrome uppercase 0.14em; ≥1.4cqw floor.
...
- **Mochiy Pop One + IBM Plex Mono via Google Fonts**; Noto Sans SC is the CJK fallback (the lowercase-display signal has no CJK equivalent — the two-register color system carries the identity, per the source).
