...
import type { BlurShape, Shape } from "@/lib/editor/doc";
...
/**
 * ぼかし（ガウスぼかし）ツール。モザイクの姉妹で、同じドラッグ操作で領域を指定する。
 * ドラッグ中のプレビューは軽い半透明の枠で表し、確定後の実ぼかしは
 * render.ts の buildBlurNode がベース画像から行う（毎フレーム再ぼかしは重い）。
 */
export class BlurTool extends DragTool {
	readonly name: ToolName = "blur";
