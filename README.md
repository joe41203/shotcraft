...
- **テキストの縁取り（ハロー）**: テキスト注釈には、どんな背景でも読めるように自動で細い縁取り（ハロー）が付きます。縁の色は文字色の明るさから自動判定し（明るい文字にはダークな縁、暗い文字には白い縁）、縁の太さはフォントサイズに連動します。エディタ表示と書き出しで同じ見た目になります（フキダシ内の文字には、本体の塗りが背景になるため縁取りは付けません）。
...
- **フキダシ**: フキダシツール（`B`）でドラッグして角丸のフキダシ本体を描くと、続けてテキストを入力できます（`Enter` 確定 / `Shift+Enter` 改行 / `Esc` キャンセル）。しっぽ（三角）は本体の辺の中央から外向きに出ます。フライアウトの「しっぽ」セクションで向き（下 / 上 / 左 / 右）を複数選べ（全 OFF でしっぽなし。既定は「下」）。選択ツールで移動・リサイズでき、リサイズするとテキストが折り返して高さが追従します。ダブルクリックで文言を再編集できます。
