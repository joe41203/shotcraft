...
[![version](https://img.shields.io/badge/version-0.8.1-22c55e)](./package.json)
...
- **スポイト（画面の色を拾う）**: 色スウォッチの右にあるスポイトボタンを押すと、画面上の好きな一点をクリックしてその色を注釈の色にできます。パレットに無い色（画面内のブランドカラー、エラー表示の赤など）に合わせたいときに使います。拡張のタブから使っても**画面全体（他のウィンドウを含む）から色を拾えます**。拾った色はパレットのどれとも一致しないことが多いので、いま使っている色をスポイトボタンの下線で示します。追加の権限は必要ありません（ブラウザの機能をそのまま使います）。
- **スタイルは前回の選択を記憶**: 選んだ色・線種・矢印スタイル・文字サイズ・塗り・強度・暗さ・フキダシのしっぽの向き・クロップの比率・書き出し形式と品質・フチ（フレーム）の種類は次回エディタを開いたときにも引き継がれ、前回の設定で編集を始められます。
...
- **フチ（フレーム）3 種**: スクリーンショット全体をフレームで囲めます。白背景のページを白い資料に貼ったときなど、画像の境界が背景に溶けて「どこまでが画像か分からない」状態を防げるほか、ブログや資料に貼るときの見栄えを整えられます。ツールバーのフチボタンから **枠線 / ブラウザ / ダーク** を選ぶだけで、どのツールを使っていても切り替えられます（ツールに紐づかない画像単位の設定です）。**枠線**は太さ（細 / 標準 / 太）と色を選べるシンプルな線で、明るい資料にも暗い資料にも合わせられます。**ブラウザ**は信号機ボタンとアドレスバー付きのウィンドウ風で、アドレス欄には撮影元の URL が自動で入ります（自由に書き換え・空欄にもできます）。白い背景の資料になじみます。**ダーク**は暗いタイトルバー付きのウィンドウ風（タイトルは撮影元のページ名が初期値で、自由に変更できます）で、暗い背景の資料になじみます。フレームは画像の**外側**に足すので元の内容が隠れることはなく、その分だけ出力サイズが余白のぶん大きくなります（ブラウザ風・ダーク風は上部バーのぶん上だけ厚くなります）。クロップと併用するとクロップ後の範囲の外側に付き、角丸のウィンドウ風ではコンテンツの角も同じ曲率で丸まります。注釈やスポットライトの暗幕より前面に描かれるため、暗幕で暗くなることはありません。変更は元に戻す（undo）の対象で、選んだ種類は次にエディタを開いたときにも引き継がれます（URL・タイトルはその画像固有の内容なので引き継ぎません）。
...
- **設定はブラウザ内に保存**: 新規図形用の設定（色・線種・矢印スタイル・文字サイズ・塗り・強度・暗さ・フキダシのしっぽの向き・クロップの比率・書き出し形式と品質・フチ（フレーム）の種類）だけはブラウザ内の `storage.local` に保存し、次回の編集に引き継ぎます。設定は画像データを含まず、外部へ送信しません。
...
- **スポイト**: 色スウォッチの右のスポイトボタンを押すと画面の色を拾えます。押すと画面全体が拡大鏡の表示に変わるので、使いたい色の上でクリックしてください（`Esc` でやめられます）。拾った色は次に描く図形の色になり、図形を選択中ならその図形にも即座に反映されます。
...
新規図形用の設定（色・線種・矢印スタイル・文字サイズ・塗り・強度・暗さ・フキダシのしっぽの向き・クロップの比率・書き出し形式と品質・フチ〈フレーム〉の種類）だけは例外で、`browser.storage.local`（`style-prefs.ts`）に保存し、次回エディタを開いたときに前回の設定で始められるようにします。画像データは含めません。
...
lib/
  editor/
    ...
    style-prefs.ts         新規図形用スタイル（色・線種・文字サイズ・塗り・強度・暗さ・しっぽの向き・クロップ比率・書き出し形式/品質・フチの種類等）の storage.local 保存
    crop.ts / mosaic.ts    クロップ・モザイクの座標／ピクセル計算
    border.ts              フチ（フレーム 3 種）の正規化・4 辺の余白／寸法／オフセット／各パーツ配置の計算
...
座標変換（CSS px から画像 px への変換）、範囲選択の要素スナップ（クリック／ドラッグ判定のしきい値・要素矩形のビューポートクランプと極小要素の除外）、ドキュメントモデルと undo/redo 履歴、ズーム/フィット計算、モザイクのピクセルサイズ決定・ぼかし半径／角丸半径の決定（いずれも強度倍率の適用を含む）・スポットライト暗幕の穴矩形のクランプ・角丸／フェザー寸法・暗幕の挿入位置・暗さ（不透明度）の正規化と解決の決定、スマート消しゴムの周長 IDW ブレンド（縁の外れ値除去〈近傍メディアン＋周長全体の多数派色〉・周長サンプルの間引き・粗グリッド IDW とバイリニア拡大・幅広の写り込みが筋にならないことのゴールデンテスト）・領域矩形の範囲クランプ・弱いぼかし半径の決定、クロップの座標合成・比率拘束のリサイズ整形／中心を保った比率整形（アスペクト維持と画像範囲クランプの両立）、フチ（フレーム 3 種）の正規化（種類の判別・kind を持たない旧データの単色枠としての読み込み・太さの丸め／上限クランプ・URL／タイトルの長さ制限・未知の種類のフチなしへの解決）と同値判定、種類ごとの 4 辺の余白と外側方式の寸法計算（クロップ後寸法＋余白）・コンテンツのオフセット／クリップ矩形／角丸半径・ストロークが中心基準であることを踏まえた枠矩形の算出（枠がステージ外周にぴったり収まること）・ブラウザ／ダークの上部バー配置（信号機の等間隔とテキスト領域がバー内に収まること）、表示用 URL の短縮（クエリ・ハッシュの除去）、出力ファイル名の生成（形式に応じた拡張子）と書き出し形式・品質の正規化に加え、ステップ番号の自動採番・次番号の明示上書き（「次を1に戻す」）の解決、フキダシの折返し高さ・4 方向のしっぽ形状・複数しっぽ／しっぽなしの正規化・色の計算、線種（実線 / 破線）の解決、文字サイズ／強度／暗さ／しっぽの向き／クロップ比率のプリセット判定・正規化、スタイルフライアウトに出すセクション（線種 / 矢印 / サイズ / しっぽ / 塗り / 強度 / 暗さ / 番号 / 比率）とアンカー先ツールボタンの判定、新規図形用スタイル設定（色・線種・矢印スタイル・文字サイズ・塗り・強度・暗さ・しっぽの向き・クロップ比率・書き出し形式/品質・フチ）の正規化と同値判定、描画時の Shift 制約（角度スナップ・正方形化）の計算、図形の平行移動・複製・重ね順（z 順）変更・グループの付与／解除／複製時のグループ再採番、矢印スタイルの正規化・曲線矢印の制御点計算、複数選択の交差判定（バウンディングボックス）・グループ所属への選択拡張、整列スナップの吸着計算（x/y 各軸で 9 通りの端の組合せから最近傍候補を選び、吸着量とガイド線区間を返す）、ペン・マーカーの手ブレ補正（点列の間引き）について、[Vitest](https://vitest.dev/) でユニットテストを用意しています。
...
<details>
<summary>フチ（フレーム）の種類・出力サイズ・URL の扱い</summary>

フチはツールに紐づかない**画像単位の設定**で、ツールバーのフチボタン（フライアウト）から種類を選びます。

| 種類 | 見た目 | 向いている貼り先 | 追加で選べるもの |
| --- | --- | --- | --- |
| 枠線 | 単色の線で囲む | 明るい資料・暗い資料のどちらでも | 太さ（細 2px / 標準 6px / 太 12px）と色 |
| ブラウザ | 信号機ボタン＋アドレスバー付きのウィンドウ風（角丸） | 白背景の資料・ブログ | アドレスバーに出す URL |
| ダーク | 暗いタイトルバー付きのウィンドウ風（角丸） | 暗い背景の資料・スライド | タイトルバーに出す文字 |

フレームはコンテンツの**外側**に足します。したがって出力画像はクロップ後の寸法より大きくなり、増える量は種類ごとの余白で決まります（枠線は 4 辺とも太さのぶん、ブラウザ風・ダーク風は上部バーのぶん上辺だけ厚くなります）。元の内容がフレームに隠れることはありません。クロップと併用すると、クロップ後の範囲の外側に付きます。角丸を持つ種類（ブラウザ / ダーク）では、中のコンテンツも同じ曲率で丸めて描くので角がはみ出しません。フレームの種類を変えると出力寸法が変わるため、変更後はビューを自動で再フィットします（拡大表示中でも、上部バーのぶん押し下がった画像が画面外に隠れないようにするため）。

**URL・タイトルの扱い**: ブラウザ風を選ぶと、アドレスバーには撮影元ページの URL が初期値として入ります。表示時に**クエリ文字列（`?...`）とハッシュ（`#...`）は落とします**（トークンやセッション ID が意図せず写り込むのを防ぐため）。フライアウトの入力欄で自由に書き換えたり、空欄にしてバーだけを出したりできます。ダーク風のタイトルも同様に、撮影元のページ名が初期値で自由に変更できます。これらの文字列は**その画像固有の内容なので設定として保存しません**（前の画像の URL が次のキャプチャに引き継がれてしまうため）。記憶するのは種類と、枠線の太さ・色だけです。

重ね順は最前面です。スポットライトの暗幕・注釈のいずれよりも上に描かれるため、暗幕でフレームが暗くなることはありません。角丸の種類では四隅が透明になるため、PNG / WebP では透過のまま、JPEG では白背景合成の対象になります。

フチの設定は `EditorDoc` のフィールドとして持つため、変更は**元に戻す（undo）の対象**です。同時に種類が `storage.local` にも記憶され、次に開いた画像は前回と同じ種類で始まります（編集内容を復元した場合は、その画像自身の設定を優先します）。
</details>
