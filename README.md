...
- **遅延キャプチャ（3秒 / 5秒）**: ポップアップ下部の「遅延」（即時 / 3秒 / 5秒）を選ぶと、その秒数だけ待ってから撮影します。表示範囲・ページ全体のキャプチャに共通で効きます（範囲選択はオーバーレイを出す前に待つだけで用途が薄いため対象外です）。ホバーメニューやツールチップを出した状態で撮りたいときに使います。待機はバックグラウンドで行うため、ポップアップは選択後すぐ閉じて構いません。
