...
- **ぼかし**: モザイクの姉妹ツール（`U`）。ドラッグした矩形のベース画像をガウスぼかしにして情報を伏せます。ぼかしの強さは範囲の大きさから自動で決まり、移動・リサイズすると都度計算し直します。
- **スポットライト（暗幕）**: ドラッグで囲んだ領域だけを明るく残し、それ以外の画像全体を暗くして視線を誘導します（`O`）。複数置いても暗幕は 1 枚のまま、囲んだ数だけ明るい穴が増えます。選択で移動・リサイズできます。
...
- **ぼかし**: ぼかしツール（`U`）でドラッグした矩形範囲のベース画像をガウスぼかしにします。モザイクの姉妹で、強さ（半径）は範囲の大きさから自動で決まり、移動・リサイズすると計算し直します。選択ツールで移動・リサイズ・削除できます（回転はできません）。
- **スポットライト（暗幕）**: スポットライトツール（`O`）でドラッグして囲んだ領域だけを明るく残し、それ以外の画像全体を暗くして視線を誘導します。複数置いても暗幕は 1 枚のまま明るい穴が増えます。選択ツールで移動・リサイズ・削除できます（回転はできません）。
...
lib/
  editor/
    ...
    blur.ts / spotlight.ts ぼかし半径・スポットライト暗幕の純粋計算
...
座標変換（CSS px から画像 px への変換）、ドキュメントモデルと undo/redo 履歴、ズーム/フィット計算、モザイクのピクセルサイズ決定・ぼかし半径の決定・スポットライト暗幕の穴矩形のクランプ、クロップの座標合成、出力ファイル名の生成に加え、ステップ番号の自動採番、遅延キャプチャの待機時間の正規化、フキダシの折返し高さ・しっぽ形状・色の計算、線種（実線 / 破線）の解決について、[Vitest](https://vitest.dev/) でユニットテストを用意しています。
