...
[![version](https://img.shields.io/badge/version-0.1.2-22c55e)](./package.json)
...
- **13 種類のツール**: 選択・矢印・矩形・楕円・スポットライト・テキスト・ペン・蛍光マーカー・ステップ・フキダシ・モザイク・ぼかし・クロップ。
...
- **設定はブラウザ内に保存**: 新規図形用の設定（色・線種）だけはブラウザ内の `storage.local` に保存し、次回の編集に引き継ぎます。設定は画像データを含まず、外部へ送信しません。
...
- **選択・変形**: 選択ツールで図形をクリックすると、周囲のハンドルで移動・リサイズ・回転できます（モザイク・ぼかし・スポットライトは回転できません。テキストは四隅ハンドルのみで縦横比を保った拡大縮小になり、回転はしません。ステップの丸バッジは固定サイズで、移動と削除のみできます）。
...
座標変換（CSS px から画像 px への変換）、ドキュメントモデルと undo/redo 履歴、ズーム/フィット計算、モザイクのピクセルサイズ決定・ぼかし半径／角丸半径の決定・スポットライト暗幕の穴矩形のクランプ・角丸／フェザー寸法・暗幕の挿入位置の決定、クロップの座標合成、出力ファイル名の生成に加え、ステップ番号の自動採番、遅延キャプチャの待機時間の正規化、フキダシの折返し高さ・しっぽ形状・色の計算、線種（実線 / 破線）の解決について、[Vitest](https://vitest.dev/) でユニットテストを用意しています。
...
色は、新しく描く図形にのみ適用されます。既存図形への適用は今後対応予定です。線の太さは 4px 固定で、選択 UI はありません（過去に太さを変えて保存した図形は、その太さのまま描画されます）。
</details>
