...
- **バグ報告テンプレートのコピー**: ツールバー右端の「バグ報告」ボタンで、issue にそのまま貼れる Markdown テンプレートをクリップボードへコピーします。テンプレートには「発生した問題 / 再現手順 / 期待する動作 / スクリーンショット（貼り付け案内）/ 環境」の見出しと、環境表（撮影元ページの URL・タイトル、撮影日時、画面サイズ、画像サイズ、ブラウザの `userAgent`、shotcraft のバージョン）が入ります。値が取れなかった項目（`chrome://` などで URL が取れない場合など）はその行を出しません。テンプレート生成は完全にローカルで行い、コピーするのは自分のクリップボードだけです（外部へは送信しません）。画像はテキストと別種のデータなので含めず、本文の案内どおり「コピー」ボタンで貼り付けます。
...
- **バグ報告テンプレートもローカル完結**: 「バグ報告」ボタンが作る Markdown には撮影元ページの URL・タイトルやブラウザの `userAgent` などが含まれますが、これらはあなたのクリップボードにコピーされるだけで、あなたが自分で issue などへ貼り付けるまでどこにも送信されません。テンプレートの組み立ては拡張内（完全ローカル）で行い、外部リクエストは発生しません。
...
- **バグ報告**: ツールバー右端の「バグ報告」ボタンを押すと、issue にそのまま貼れる Markdown テンプレート（発生した問題・再現手順・期待する動作・スクリーンショット貼り付け案内・環境表）をクリップボードへコピーします。環境表には撮影元ページの URL・タイトル、撮影日時、画面サイズ、画像サイズ、ブラウザの `userAgent`、shotcraft のバージョンが入り、取れなかった項目の行は出しません。画像はテキストと同時にクリップボードへ持てないため含みません。テンプレートを貼った後、続けて「コピー」ボタン（または `Ctrl` / `Cmd` + `C`）で画像をコピーし、「スクリーンショット」の位置に貼り付けてください。
